...
<template>
  <div>
    <div class="mobile-row d-flex p-t-12" :style="{ backgroundColor: rowBGColor }">
      <div class="d-flex align-items-center m-l-30 m-b-12">
        <slot name="mobile-collapse-icon">
          <MobileCollapseIcon class="m-r-12 cursor-pointer" :is-collapsed="showCollapsedCells"
            @click="toggleCollapsedCells" />
        </slot>
        <slot name="checkbox">
          <Checkbox v-if="options.hasCheckbox" class="m-r-9" :is-checked="isChecked" :value="record.id" :disabled="!record.isSelectable && record.isSelectable !== undefined
    ...
    <transition name="animate-height">
      <div v-if="showCollapsedCells" class="p-18 w-100 collapsed-section">
        <div v-for="cell in collapsedRowCells" :key="cell.id"
          class="d-flex justify-content-between align-items-center m-b-24">
          <p class="heading-6-light">
            {{ cell.name }}
          </p>
          <div class="text-right p-small-bold" @click="cell.isClickable && entityClicked($event, cell.actionId)">
