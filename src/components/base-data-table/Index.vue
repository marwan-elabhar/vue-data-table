<script>
  ...
  export default {
    components: {
      ...
      Empty,
      Loading,
      ActionsGroupAlert: () => import("./components/ActionsGroupAlert.vue"),
    },
    props: {
      content: {
        type: Array,
...
<template>
  <div class="w-100">
    ...
    <div
      v-else
      class="w-100 radius-8 bg-base-white overflow-x-auto primary-scroll hidden-scroll table-container"
    >
      <Header
        v-if="mergedOptions.showHeader"
        :cells="sortedCells"
        ...
        @update-checkbox="toggleSelectAll"
    ...
    <slot v-if="options.showActionGroupAlert" name="bulk-actions-alert">
      <transition name="fadeUp">
        <ActionsGroupAlert
          v-if="selectedRows.length > 1 && actions.length"
          :actions="actions"
